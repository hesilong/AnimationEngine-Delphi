...
Type
  ...
  TLogicWithStar = class(TBaseLogic)
    protected
     ...
     FFullStarName : String;
     FHalfStarName : String;
     FEmptyStarName : String;
...
function TLogicWithStar.UpdateASingleStarStatus(AStarIndex: Integer): TEngine2DSprite;
var
  ...
  if LSprite <> nil then
    begin
       ...
       if AStarIndex <= LDivV then
         LImage.ImageConfig.SetNormalImg(FFullStarName) else
         begin
           LModeV := FStarCount mod 2;
           if AStarIndex = LDivV+1 then
             begin
               if LModeV = 1 then
                 LImage.ImageConfig.SetNormalImg(FHalfStarName) else
               if LModeV = 0 then
                 LImage.ImageConfig.SetNormalImg(FEmptyStarName);
             end else
             begin
               LImage.ImageConfig.SetNormalImg(FEmptyStarName);
             end;
